...
/*
    This fragment shader is used to calculate and animate position, size, and color for the particle simulation.
    This data is then output as three separate data textures and subsequently passed to the main shaders.
*/


// This function animates the particles towards (0,0,0)
...
void main() {
    ...
    float part_id = texture(uPos, vUv).a;
        ...
        float x1 = rad*cos(theta)*(part_id/count)*sin(uTime*speed);
        float y1 = rad*sin(theta)*(part_id/count)*sin(uTime*speed);
        float z1 = 0.0;
        z1 += pow(((part_id/count)*3.0*rad), strength)*sin(uTime*speed);
        ...
        final_color.r += 0.2*sin(uTime);
        final_color.g += 0.3*sin(uTime*0.5);
        final_color.b += 0.4*sin(uTime*0.8);
        fboColor = vec4(final_color, 1.0);
    
    
        float distance_multz = distance(pos, vec3(0.0,0.0,-7.0) + uGradientOffset);
