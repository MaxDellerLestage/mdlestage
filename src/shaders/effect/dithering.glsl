...
vec3 orderedDither(vec2 uv, vec3 lum) {
    ...
    if (matrixSize == 8.0) {
        ...
        threshold = bayerMatrix8x8[y * 8 + x]-0.9;
    }

    color.rgb += threshold * 0.17;
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    vec2 new_center = vec2(center.x + cos(time*0.3)*0.1, center.y + sin(time*0.05)*0.1);

    float dist = distance(uv, new_center);
    float mask = smoothstep(radius, radius-0.03, dist);

    vec3 dithered = orderedDither(uv, color.rgb);

    color.rgb = mix(color.rgb, dithered, mask);
